...
import { LOCALES, DEFAULT_LOCALE_CODE, getLocale, format } from "./i18n";
...
function LanguageSelect({ localeCode, setLocaleCode, label }) {
  return (
    <label className="relative flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 py-2 pl-3 pr-8 text-sm">
      <Languages className="h-3.5 w-3.5 shrink-0 text-zinc-500" aria-hidden="true" />
...
function VehicleCard({ vehicle, onOpen }) {
  ...
  const fa = vehicle.translations?.fa;
  const displayName = showFa && fa?.name ? fa.name : vehicle.name;
  const displayDesignation = showFa && fa?.designation ? fa.designation : vehicle.designation;
  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        {/* Per-card Persian translation toggle — flips the visible name and
            designation to vehicle.translations.fa, independent of the
            interface language selected up in the header. */}
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
          ...
          title={fa ? "فارسی" : "ترجمه فارسی هنوز آماده نیست"}
          aria-pressed={showFa}
          className={`absolute bottom-2 right-2 flex items-center gap-1 rounded-md border px-1.5 py-1 text-[10px] font-mono transition-colors ${
            showFa
              ? "border-emerald-400/40 bg-emerald-400/10 text-emerald-300"
              : "border-white/10 bg-zinc-950/70 text-zinc-400 hover:border-white/30 hover:text-zinc-200"
          } ${!fa ? "opacity-50" : ""}`}
        >
      ...
      <div className="p-3.5" dir={showFa && fa ? "rtl" : "ltr"}>
        <p className="font-mono text-xs text-zinc-500">{vehicle.nation}</p>
        <h3
          className="text-sm font-semibold leading-snug text-zinc-100"
          style={showFa && fa ? { fontFamily: "'Vazirmatn', 'Noto Sans Arabic', sans-serif" } : undefined}
        >
          {displayName}
        </h3>
        <p
...
function EmptyState({ t }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p className="text-sm text-zinc-400">{t.noResultsTitle}</p>
      <p className="mt-1 text-xs text-zinc-600">{t.noResultsSubtitle}</p>
    </div>
  );
}

function DossierModal({ vehicle, onClose, t }) {
  const [blueprint, setBlueprint] = useState(false);
  const [showFa, setShowFa] = useState(false);
  const Icon = DOMAIN_META[vehicle.domain]?.icon ?? Shield;
  const fa = vehicle.translations?.fa;
  const translated = showFa && fa;

  const displayName = translated && fa.name ? fa.name : vehicle.name;
  const displayDesignation = translated && fa.designation ? fa.designation : vehicle.designation;
  const displayRole = translated && fa.role ? fa.role : vehicle.role;
  const displayBrief = translated && fa.laymansBrief ? fa.laymansBrief : vehicle.laymansBrief;
  const displaySpecs = translated && fa.specs ? fa.specs : vehicle.specs;
  const specLabelFor = (key) =>
    translated && fa.specLabels?.[key] ? fa.specLabels[key] : formatLabel(key);
  const faFont = { fontFamily: "'Vazirmatn', 'Noto Sans Arabic', sans-serif" };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-video overflow-hidden border-b border-white/10">
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-2">
            <button
              ...
              title={fa ? undefined : t.translationUnavailable}
              aria-pressed={showFa}
            ...
            >
              ...
              {showFa ? t.showOriginal : t.translateToFa}
            </button>
            ...
            >
              ...
              {blueprint ? t.exitBlueprintMode : t.blueprintMode}
            </button>
          </div>
        ...
        <div className="space-y-6 p-5 sm:p-7" dir={translated ? "rtl" : "ltr"} style={translated ? faFont : undefined}>
          {showFa && !fa && (
          ...
          <div>
            ...
            <h2 className="mt-1 text-2xl font-semibold text-zinc-50">{displayName}</h2>
            <p className="text-sm text-zinc-500">{displayDesignation}</p>
            <p className="mt-2 text-sm text-zinc-400">
              {displayRole} — manufactured by {vehicle.manufacturer}, introduced {vehicle.yearIntroduced}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-amber-300">{t.laymansBriefTitle}</h3>
            </div>
            <p className="text-sm leading-relaxed text-zinc-300">{displayBrief}</p>
          </div>

          <div>
            <h3 className="mb-3 text-sm font-semibold text-cyan-300">{t.specsTitle}</h3>
            <div className="grid grid-cols-1 gap-x-8 sm:grid-cols-2">
              {Object.entries(displaySpecs).map(([key, value]) => (
                <SpecRow key={key} label={specLabelFor(key)} value={value} />
              ))}
            </div>
          </div>
        </div>
...
function Header({ resultCount, total, t, localeCode, setLocaleCode }) {
  return (
        ...
        <div className="flex items-center gap-2">
          <div className="hidden items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/5 px-3 py-1.5 sm:flex">
            ...
            <span className="font-mono text-xs text-cyan-300">
              {format(t.records, { count: resultCount, total })}
            </span>
          </div>
          <LanguageSelect localeCode={localeCode} setLocaleCode={setLocaleCode} label={t.languageLabel} />
        </div>
      </div>
    </header>
  );
...
function CommandFilterBar({ queryInput, setQueryInput, domain, setDomain, nation, setNation, era, setEra, nations, eras, t }) {
  // Internal values stay the canonical English keys used by the data schema
  // (vehicle.domain is always "Land" | "Air" | "Sea") — only the label shown
  // to the user is translated.
  const domains = [
    { value: "All", label: t.filterAll },
    { value: "Land", label: t.filterLand },
    { value: "Air", label: t.filterAir },
    { value: "Sea", label: t.filterSea },
  ];
  return (
    <div className="border-b border-white/10 bg-zinc-950/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6 lg:flex-row lg:items-center">
        <div className="relative w-full flex-1 lg:w-auto">
          <Search className="absolute rtl:right-3 rtl:left-auto left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500" />
          <input
            value={queryInput}
            onChange={(e) => setQueryInput(e.target.value)}
            placeholder={t.searchPlaceholder}
            className="w-full rounded-lg border border-white/10 bg-white/5 py-2 pl-9 pr-3 text-sm text-zinc-200 placeholder-zinc-600 outline-none transition-colors focus:border-amber-400/50 rtl:pl-3 rtl:pr-9"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5">
          {domains.map((d) => (
            <FilterPill
              ...
              icon={d.value === "All" ? Radar : DOMAIN_META[d.value]?.icon}
              label={d.label}
            />
          ))}
        </div>

        <SelectField label={t.nationLabel} value={nation} options={nations} onChange={setNation} />
        <SelectField label={t.eraLabel} value={era} options={eras} onChange={setEra} />
      </div>
    </div>
  );
}
...
export default function DeclassifiedArsenal() {
  ...
  // Interface language. Persisted across visits; falls back to the
  // browser's language if it happens to match one of the 21 supported
  // locales, otherwise defaults to English.
  const [localeCode, setLocaleCode] = useState(() => {
    try {
      const saved = window.localStorage.getItem("arsenal-locale");
      if (saved && LOCALES.some((l) => l.code === saved)) return saved;
    } catch {
      /* localStorage unavailable (e.g. private browsing) — fall through */
    }
    const browser = (navigator.language || "").slice(0, 2);
    return LOCALES.some((l) => l.code === browser) ? browser : DEFAULT_LOCALE_CODE;
  });
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("arsenal-locale", localeCode);
    } catch {
      /* ignore */
    }
    document.documentElement.lang = localeCode;
    document.documentElement.dir = locale.dir;
  }, [localeCode, locale.dir]);
  ...
  return (
    ...
    >
          ...
          <>
            ...
            {visibleCount < filtered.length && (
              <div className="mt-8 flex justify-center">
                ...
                >
                  {format(t.loadMore, { n: Math.min(PAGE_SIZE, filtered.length - visibleCount) })}
                </button>
              </div>
            )}
          </>
      ...
      <AnimatePresence>
        {selected && <DossierModal vehicle={selected} onClose={() => setSelected(null)} t={t} />}
      </AnimatePresence>
    </div>
  );
}
