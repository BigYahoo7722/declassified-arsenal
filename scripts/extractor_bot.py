...
-------------------------------------------------------------------------
    {
      ...
      "manufacturer":   str,
      "role":           str,
      "thumbnail":      str,   Wikipedia's lead image for the page, via the
                                pageimages API (see fetch_thumbnail), or ""
                                for pages with no image; the frontend falls
                                back to a hangar placeholder image when empty
      "laymansBrief":   str,
      "tags":           [str],
      "specs":          {str: str},
      "translations":   {str: {...}}, optional — present only when
                                --translate-fa was used and the LLM call
                                succeeded for this record. Currently only
                                the "fa" (Persian) key is generated:
                                  translations.fa = {
                                    name, designation, role, laymansBrief,
                                    specs: {same keys as top-level specs,
                                            translated values},
                                    specLabels: {same keys, Persian labels}
                                  }
                                See src/i18n.js and the per-card "FA" toggle
                                in src/DeclassifiedArsenal.jsx, which reads
                                exactly this shape.
    }
...
-------------------------------------------------------------------------
    ...
    # Also generate a Persian translation of each record (needs an LLM key):
    python extractor_bot.py --category "Main battle tanks" --translate-fa
...
def fetch_thumbnail(session: requests.Session, title: str) -> str:
    """Look up the page's lead image via MediaWiki's `pageimages` prop and
    return a ready-to-use thumbnail URL sized for the card/dossier UI, or ""
    if the page has no image (some stub articles don't).

    This hotlinks directly to Wikimedia's thumbnail servers rather than
    downloading and re-hosting the file. That's fine at the request volumes
    this crawler runs at (one lookup per vehicle, once a month, with the
    same politeness delay as every other API call here) — but if this ever
    grows into a much higher-traffic deployment, download the image once,
    store it in public/images/, and serve it from there instead, both to be
    a better citizen of Wikimedia's infrastructure and to keep the site
    working even if a file is later renamed or deleted upstream.
    """
    data = wiki_api_get(session, {
        "action": "query", "titles": title, "prop": "pageimages",
        "piprop": "thumbnail", "pithumbsize": 1024, "redirects": 1,
    })
    pages = data.get("query", {}).get("pages", {})
    for page in pages.values():
...
TRANSLATE_FA_PROMPT_TEMPLATE = """Translate the following military vehicle record into Persian (Farsi) for a \
general-audience Persian-speaking reader. Keep proper nouns (vehicle name, manufacturer) transliterated \
naturally into Persian where that's how they're normally written; keep well-known NATO reporting names and \
model numbers as-is if transliterating them would be unrecognizable. Numbers, units, and the dict *keys* in \
"specs" must stay exactly as given — translate only the spec *values* and the spec *labels*.

Vehicle name: {name}
Designation: {designation}
Role: {role}
Layman's brief: {laymans_brief}
Specs (key: value): {specs_json}

Respond with ONLY a single JSON object, no markdown fences, no commentary, in exactly this shape:
{{
  "name": "<Persian name>",
  "designation": "<Persian designation>",
  "role": "<Persian role>",
  "laymansBrief": "<Persian translation of the brief, 2-4 sentences>",
  "specs": {{"<original spec key>": "<translated spec value>", ...}},
  "specLabels": {{"<original spec key>": "<Persian label for that spec>", ...}}
}}

Every key present in the input "specs" object must appear in both output "specs" and "specLabels", with no \
...
def generate_persian_translation(
    session: requests.Session,
    vehicle: Dict[str, Any],
    provider: str,
    model: str,
) -> Optional[Dict[str, Any]]:
    """Ask the LLM for a Persian translation of the record's display fields,
    returned as the `translations.fa` block the frontend's per-card "FA"
    toggle reads (see src/i18n.js and the DossierModal / VehicleCard
    components in src/DeclassifiedArsenal.jsx). Returns None — and only
    logs a warning — on any failure: no API key, a network error, or a
    malformed JSON response should never crash the batch or block the
    English record itself from being written."""
    prompt = TRANSLATE_FA_PROMPT_TEMPLATE.format(
        name=vehicle["name"],
        designation=vehicle["designation"],
        role=vehicle["role"],
        laymans_brief=vehicle["laymansBrief"],
        specs_json=json.dumps(vehicle["specs"], ensure_ascii=False),
    )
    try:
        caller = _call_anthropic if provider == "anthropic" else _call_openai
        raw = caller(session, prompt, model)
        # Strip ```json ... ``` fences in case the model wraps its answer
        # despite being asked not to.
        cleaned = re.sub(r"^```(?:json)?\s*|\s*```$", "", raw.strip(), flags=re.MULTILINE).strip()
        translation = json.loads(cleaned)

        required = {"name", "designation", "role", "laymansBrief", "specs", "specLabels"}
        if not required.issubset(translation.keys()):
            log.warning("Persian translation for %r is missing keys %s; discarding",
                        vehicle["name"], required - translation.keys())
            return None
        return translation
    except Exception as exc:  # noqa: BLE001 — deliberately broad, see docstring
        log.warning("Persian translation failed for %r (%s); record will ship without translations.fa",
                    vehicle["name"], exc)
        return None
...
def process_title(
    ...
    try:
        ...
        thumbnail = fetch_thumbnail(session, title)
        ...
        # Persian content translation is independent of use_llm/--no-llm:
        # it needs its own opt-in (--translate-fa) since it's an extra LLM
        # call per record and roughly doubles this step's runtime.
        if translate_fa:
            fa = generate_persian_translation(session, vehicle, provider, model)
            if fa:
                vehicle["translations"] = {"fa": fa}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--translate-fa", dest="translate_fa", action="store_true",
                         help="Also generate a Persian (Farsi) translation of each record's "
                              "display fields via the LLM, stored under vehicle.translations.fa")
    parser.set_defaults(use_llm=True, translate_fa=False)
    return parser
...
def main() -> None:
    ...
    if args.use_llm and not os.environ.get(key_var):
    ...
    if args.translate_fa and not os.environ.get(key_var):
        log.warning("%s is not set — --translate-fa was requested but every translation call will fail; "
                    "records will simply ship without a translations.fa block.", key_var)
    ...
    for i, title in enumerate(titles, start=1):
        ...
        record = process_title(session, title, args.use_llm, args.provider, model, args.translate_fa)
        if record is None:
            failed += 1
            continue
